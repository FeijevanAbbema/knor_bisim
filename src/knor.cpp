...
handleOptions(int &argc, char**& argv)
{
    try {
        ...
        opts.add_options()
            ...
            ("bisim-min", "Solve the game and report how much it can be minimised")
            ;
...
struct sig_part {
    ...
    bool operator==(const sig_part& rhs) const {
        return this->value == rhs.value && this->level == rhs.level;
    };
    bool operator<(const sig_part& rhs) const {
...
std::map<std::vector<bool>, MTBDD> getStatesList(MTBDD& bdd, uint32_t state_bits, std::vector<bool> curpath, uint32_t offset) {
    ...
    if (low != sylvan_false) {
        ...
        if (low_var < state_bits + offset) {
            // Still in the state bdd part
            curpath.push_back(false);
            std::map<std::vector<bool>, MTBDD> low_res = getStatesList(low, state_bits, curpath, offset);
            result.insert(low_res.begin(), low_res.end());
            curpath.pop_back();
        } else {
            // The BDD is now the BDD of a single state
            curpath.push_back(false);
            result.insert(std::make_pair(curpath, low));
            curpath.pop_back();
        }
    }
    if (high != sylvan_false) {
        uint32_t high_var = mtbdd_getvar(high);
        if (high_var < state_bits + offset) {
            // Still in the state bdd part
            curpath.push_back(true);
            std::map<std::vector<bool>, MTBDD> high_res = getStatesList(high, state_bits, curpath, offset);
            result.insert(high_res.begin(), high_res.end());
            curpath.pop_back();
        } else {
            // The BDD is now the BDD of a single state
            curpath.push_back(true);
            result.insert(std::make_pair(curpath, high));
            curpath.pop_back();
        }
    }
    return result;
...
int
current_in_path(MTBDD current, const std::vector<path_part>& path, uint32_t offset_trans, uint32_t offset_strat) {
    uint32_t var = mtbdd_getvar(current);
    for (int i = 0; i < (int) path.size(); i++) {
        if (path[i].var - offset_strat == var - offset_trans) {
            return i;
        }
    }
    return -1;
...
get_trans_block(std::vector<std::vector<std::vector<bool>>> blocks, SymGame sym, std::vector<path_part> path, uint32_t offset_strat, const std::vector<bool>& state_path) {
    ...
    // Now we are at the next state: record path to next state
    std::vector<std::vector<bool>> next_states;
    for (auto bdd : next_states_bdd) {
        std::vector<bool> next_state;
        current = bdd;
        for (int i = 0; i < sym.statebits; i++) {
            uint32_t prev_var = mtbdd_getvar(current);
            MTBDD low = mtbdd_getlow(current);
            MTBDD high = mtbdd_gethigh(current);
            if (low == sylvan_false) {
                if (high == sylvan_false) {
...
computeSignature (MTBDD bdd, SymGame sym, const std::vector<std::vector<std::vector<bool>>>& blocks, std::vector<path_part> curpath, uint32_t offset, const std::vector<bool>& state_path) {
    ...
    if (high == sylvan_false || high == sylvan_true) {
        ...
        if (high == sylvan_false) {
            high_sig.value = std::vector<int>(1, -1);
        } else {
            high_sig.value = get_trans_block(blocks, sym, curpath, offset, state_path);
        }
        result.push_back(high_sig);
    } else if (mtbdd_getvar(high) >= signature_level + offset) {
        sig_part high_sig{get_trans_block(blocks, sym, curpath, offset, state_path), cur_var};
        result.push_back(high_sig);
    } else {
        curpath.push_back(path_part{true, cur_var});
        std::vector<sig_part> high_sig = computeSignature(high, sym, blocks, curpath, offset, state_path);
        result.insert(result.end(), high_sig.begin(), high_sig.end());
        curpath.pop_back();
    }
    return result;
}
...
minimiseBDD(SymGame sym) {
    ...
    for(auto & it : path_states_map) {
        state_paths.push_back(it.first);
        states_bdds.push_back(it.second);
    }
    ...
    // main process
    while (no_old_blocks != no_blocks) {
        no_old_blocks = no_blocks;
        new_blocks = blocks;
        ...
        for(size_t i = 0; i < blocks.size(); i++) {
            ...
            for (size_t j = 0; j < blocks[i].size(); j++) {
                // Compute signature of state
                std::vector<path_part> empty_path_parts;
                std::vector<sig_part> signature = computeSignature(path_states_map[blocks[i][j]], sym, blocks, empty_path_parts, offset, blocks[i][j]);
